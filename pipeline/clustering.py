...
MAX_PLAUSIBLE_KM = 15.0  # beyond this, two reports cannot be the same incident


def geographic_similarity(r: dict, uir: dict) -> float:
    """Geographic similarity with a hard far-distance penalty.
    0–5 km: linear decay 1.0→0.0
    5–15 km: small residual (0.0–0.2)
    >15 km: 0.0
    Unknown coords: 0.5 (neutral — distinct from 'far apart')
    """
    ...
    if d_km <= 5.0:
        return 1.0 - (d_km / 5.0)
    if d_km >= MAX_PLAUSIBLE_KM:
        return 0.0
    return max(0.0, (MAX_PLAUSIBLE_KM - d_km) / (MAX_PLAUSIBLE_KM - 5.0)) * 0.2
...
class IncidentClusterEngine:
    ...
    def process_report(self, report: dict) -> dict:
        ...
        r_lat, r_lng = report.get('lat'), report.get('lng')
        for uir in self.active_uirs:
            if uir['status'] != 'active':
                continue

            # Hard geographic pre-filter: reports >15 km apart with both
            # coords resolved cannot be the same incident, regardless of
            # how similar the text or timing looks.
            u_lat, u_lng = uir.get('lat'), uir.get('lng')
            if (r_lat is not None and u_lat is not None
                    and haversine_km(r_lat, r_lng, u_lat, u_lng) > MAX_PLAUSIBLE_KM):
                continue
