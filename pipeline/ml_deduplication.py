...
MAX_PLAUSIBLE_KM = 15.0  # beyond this, treat as definitely-different incident
...
def _geo_sim(r_a: dict, r_b: dict) -> float:
    ...
    if la is None or lb is None:
        return 0.5  # unknown — neutral, distinct from "far apart" (0.0)
    d = _haversine_km(la, loa, lb, lob)
    if d <= 5.0:
        return 1.0 - (d / 5.0)
    if d >= MAX_PLAUSIBLE_KM:
        return 0.0
    # 5 km → 0.0, 15 km → strongly negative-leaning via decay
    return max(0.0, (MAX_PLAUSIBLE_KM - d) / (MAX_PLAUSIBLE_KM - 5.0)) * 0.2
...
class PairGenerator:
    ...
    def _hard_negatives(self, reports: list) -> list:
        ...
        # D) High semantic similarity but far apart — the exact case that
        # currently produces false "possible duplicate" links across the country
        same_type_reports: dict = {}
        for r in reports:
            same_type_reports.setdefault(r.get('incident_type', ''), []).append(r)
        for itype, reps in same_type_reports.items():
            geo_reps = [r for r in reps if r.get('lat') is not None][:60]
            sampled_pairs = []
            for i in range(len(geo_reps)):
                for j in range(i + 1, len(geo_reps)):
                    r1, r2 = geo_reps[i], geo_reps[j]
                    if r1['_ground_truth']['incident_id'] == r2['_ground_truth']['incident_id']:
                        continue
                    d = _haversine_km(r1['lat'], r1['lng'], r2['lat'], r2['lng'])
                    if d > 20.0:
                        sampled_pairs.append((r1, r2, 0))
            for p in random.sample(sampled_pairs, min(80, len(sampled_pairs))):
                pairs.append(p)
...
class MLDeduplicator:
    ...
    def _to_frame(self, rows):
        """Wrap feature rows in a DataFrame so models keep consistent feature names."""
        if _PANDAS:
    ...
    def predict_proba(self, r_a: dict, r_b: dict) -> float:
        ...
        if self._clf is not None:
            feat = self._to_frame([self.extract_features(r_a, r_b)])
            try:
